...
for k = 1: n - 1
  ...
  % determnar o elemento de valor absoluto na coluna
  for q = k: n
    % achou o maior valor da coluna!
    if abs(A(q, k)) == max(abs(A(:, k)))
       % trocar as linhas da matriz de coeficientes
       aux = A(q, :);
       A(q, :) = A(k, :);
       A(k, :) = aux;

      %trocar as linhas do vetor solução
      aux = b(q);
      b(q) = b(k);
      b(k) = aux;
